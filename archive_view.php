...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['vote_val'])) {
        ...
        header("Location: archive_view.php?id=$upload_id"); exit;
    }
    ...
    if (isset($_POST['comment_body'])) {
        ...
        header("Location: archive_view.php?id=$upload_id#comments"); exit;
    }
    ...
    if (isset($_POST['vote_comment_id'])) {
        ...
        header("Location: archive_view.php?id=$upload_id#c-$cid"); exit;
    }
}
...
<html>
<head>
    <title>ARCHIVE // <?= htmlspecialchars($upload['title'] ?: 'FILE_'.$upload_id) ?></title>
    ...
    <style>
        ...
        .btn-dl { background: #1a1a1a; border: 1px solid #c678dd; color: #c678dd; padding: 10px 20px; text-decoration: none; font-weight: bold; display: inline-block; }
        .btn-dl:hover { background: #c678dd; color: #000; }
    </style>
</head>
<body class="<?= $theme_cls ?? '' ?>">
    <div class="container">
        ...
        <div id="comments" style="margin-bottom: 50px;">
            <h3 style="color: #c678dd; border-bottom: 1px dashed #333; padding-bottom: 10px;">DATA ANNOTATIONS (<?= count($all_comments) ?>)</h3>
                        ...
                        echo "<div class='c-row' id='c-{$c['id']}' style='margin-left: {$pad}px;'>
                                ...
                                <div class='c-body'>
                                    ...
                                    <div style='margin-top:5px; text-align:right; font-size:0.7rem;'>
                                        ...
                                        <a href='archive_view.php?id={$c['upload_id']}&reply={$c['id']}#reply_box' style='color:#6a9c6a; text-decoration:none; margin-left:10px;'>[ REPLY ]</a>
                                    </div>
                                </div>
                              </div>";
        ...
        <div id="reply_box" style="background: #111; padding: 20px; border: 1px solid #333;">
            <?php 
                ...
                if($rep_id) echo "<div style='color:#6a9c6a; font-size:0.8rem; margin-bottom:10px;'>Replying to ID #$rep_id <a href='archive_view.php?id=$upload_id' style='color:#e06c75; text-decoration:none;'>[CANCEL]</a></div>";
            ?>
            <form method="POST">
